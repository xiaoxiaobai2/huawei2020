 ...
 * @author:  Zhang
 * @description:
 *      多线程 + 广度优先遍历 + 深度优先遍历  +  拓扑排序
 **/
public class Main {
    private static int[][] read;//二维数组，表示当前节点被访问的状态
    private static ArrayList<ArrayList<Integer>> graphTo;//To邻接表
    private static ArrayList<ArrayList<Integer>> graphFrom;//from邻接表
    private static ArrayList<ArrayList<Integer>> result;//结果集
    private static ArrayList<Integer> data;// index to id
    private static int nodeCnt;//账户数量
    private static final int THREAD_NUM =8;//线程数量
...
//        //待更新节点
        Stack<Integer> updateNode = new Stack<>();
        for (int i = 0; i < inDegrees.length; i++) {
            if (inDegrees[i]==0)
                updateNode.push(i);
        }
        while (!updateNode.isEmpty()){
            Integer node = updateNode.pop();
    ...
    /*
        深度优先遍历 每一个节点 判断是否构成环，并且环的长度 大于三，小于 7
     */
    private static void DFS(int from,int cur,int depth,ArrayList<Integer> path,int threadId,
                            ArrayList<ArrayList<ArrayList<Integer>>> nodeFromPath){
        //深度小于 5 时，进行深度搜索
        if (depth<5) {
            // 更新当前节点为已读
            read[cur][threadId] = 1;
            ...
            for (Integer v : graphTo.get(cur)) {
                ...
                if (read[v][threadId] != 1 && v > from)
                    DFS(from, v, depth + 1, path, threadId, nodeFromPath);
            }
            //重置当前节点状态
            read[cur][threadId] = 0;
            //移除最后一个节点
            ...
            path.remove(path.size()-1);
            DFS(from,cur,depth+1,path,threadId,nodeFromPath);
        }
    }
    ...
    /*
        广度优先遍历，准备好前三节
     */
    private static ArrayList<ArrayList<ArrayList<Integer>>> BFSgetFromPath(int cur){
        ...
        for (ArrayList<Integer> arrayList : fromPath2) {
            for (Integer integer : graphFrom.get(arrayList.get(0))) {
                if (integer  > cur && !integer.equals(arrayList.get(1))) {
                    ...
                    path3.add(integer);
                    path3.add(arrayList.get(0));
                    path3.add(arrayList.get(1));
                    fromPath3.add(path3);
                }
            }
        }
