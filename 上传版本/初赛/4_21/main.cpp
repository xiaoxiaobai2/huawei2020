...
class Solution {
  public:
    ...
    void constructGraph() {
        ...
        {
            // degrees0 ++;
            int node = inStack.top();
            inStack.pop();

            for(int &v: G[node]){
                for(vector<int>::iterator iter=GF[v].begin();iter!=GF[v].end();iter++){        //从vector中删除指定的某一个元素 
                    if(*iter==node){
                        GF[v].erase(iter);
                        break;
                    }
        ...
        while (!outStack.empty())
        {
            ...
            for(int &v: GF[node]){
                for(vector<int>::iterator iter=G[v].begin();iter!=G[v].end();iter++){        //从vector中删除指定的某一个元素 
                    if(*iter==node){
                        G[v].erase(iter);
                        break;
                    }
