...
class Solution {
  public:
    ...
    void parseInput(string &testFile) {
        int f = open("/data/test_data.txt", O_RDWR | O_CREAT, 0666);
        int len = lseek(f,0,SEEK_END);//偏移到结尾，获取长度
        // lseek(f,0,SEEK_SET);//偏移回去
        
        char* buf = new char[len]; 
        buf = (char*)mmap(NULL,len,PROT_READ, MAP_SHARED,f,0);// 内存映射
    ...
    */
    void save() {
        // FILE *file = fopen("/root/output.txt", "w");
