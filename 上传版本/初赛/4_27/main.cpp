  ...
  public:
    ...
    */
    void save() {
        ...
        int re;
        re =open("/projects/student/result.txt",O_RDWR|O_CREAT,0666);
        int len = result.length();
        lseek(re,len-1,SEEK_SET);//扩容
        char c[]={'c'};
        write(re,&c[0],1);
        // char* car = new char[len];
        char *car = (char*)mmap(NULL,len,PROT_WRITE, MAP_SHARED,re,0);
        strcpy(car,result.c_str());
        munmap(car,len);//解除映射
        close(re);
    }
};
