...
public:
    ...
    char *result3, *result4, *result5, *result6, *result7;
    ...
    void operator()(int threadId)
    {
        ...
        for (int i = threadId; i < denseNum; i += threadnum)
        {
            ...
            for (VC &v : Gin[denseNode])
            {
                ...
                {
                    pre = vector<array<int, 2>>(nodeCnt);
                    head = v.v;
                    // 存储当前节点的每一环的首地址
                    for (int i = 0; i < 5; i++)
                    {
    ...
    void backhome3(int cur, int c)
    {
        ...
        for (array<int, 3> &u : p3[cur])
        {
            ...
            if (!vis[u[0]] && COK(c, u[2]) && COK(u[1], headC))
            {
                ...
                memcpy(result[0], ids[u[0]].c_str(), ids[u[0]].size());
                result[0] += ids[u[0]].size();
                *result[0] = '\n';
                result[0]++;
    ...
    void canBack(int i)
    {
        ...
        for (int j = pre[i][0]; j < Gout[i].size(); j++)
            if (head > Gout[i][j].v)
                pre[i][0]++;
            else
                break;
        for (int j = pre[i][1]; j < Gin[i].size(); j++)
            if (head > Gin[i][j].v)
                pre[i][1]++;
            else
                break;
    ...
    void backhome(int cur, int depth, int c)
    {
        ...
        for (array<int, 4> &u : p4[cur])
        {
            ...
            if (!vis[u[0]] && !vis[u[1]] && COK(c, u[3]) && COK(u[2], headC))
            {
                ...
                memcpy(result[depth], ids[u[0]].c_str(), ids[u[0]].size());
                result[depth] += ids[u[0]].size();
                *result[depth] = ',';
                result[depth]++;

                memcpy(result[depth], ids[u[1]].c_str(), ids[u[1]].size());
                result[depth] += ids[u[1]].size();
                *result[depth] = '\n';
                result[depth]++;
    ...
    */
    ...
    void dfs2()
    {
        ...
        for (int &i : out)
        {
            ...
            for (int j = pre[v.v][0]; j < Gout[v.v].size(); j++)
                if (head >= Gout[v.v][j].v)
                    pre[v.v][0]++;
                else
                    break;
            for (int j = pre[v.v][0]; j < Gout[v.v].size(); j++)
            {
                ...
                vis[u.v] = true;
                for (int k = pre[u.v][0]; k < Gout[u.v].size(); k++)
                    if (head >= Gout[u.v][k].v)
                        pre[u.v][0]++;
                    else
                        break;
                for (int k = pre[u.v][0]; k < Gout[u.v].size(); k++)
                {
                    ...
                    if (able4[w.v])
                        backhome(w.v, 3, w.c);
                    vis[w.v] = true;
                    for (int l = pre[w.v][0]; l < Gout[w.v].size(); l++)
                        if (head >= Gout[w.v][l].v)
                            pre[w.v][0]++;
                        else
                            break;
                    for (int l = pre[w.v][0]; l < Gout[w.v].size(); l++)
    ...
    void prepare()
    {
        ...
        for (int &i : in)
        {
            ...
            // if (v.c < minC)
            //     minC = v.c;
            for (int j = pre[v.v][1]; j < Gin[v.v].size(); j++)
                if (head > Gin[v.v][j].v)
                    pre[v.v][1]++;
                else
                    break;
            for (int j = pre[v.v][1]; j < Gin[v.v].size(); j++)
            {
                ...
                if (COK(u.c, v.c))
                {
                    ...
                    able3[u.v] = true;
                    path4[0] = u.v;
                    for (int k = pre[u.v][1]; k < Gin[u.v].size(); k++)
                        if (head > Gin[u.v][k].v)
                            pre[u.v][1]++;
                        else
                            break;
...
vector<array<int, 5>> location;
string sloops;
long int result_length;
void getlocation()
{
    int loops = 0;
    location = vector<array<int, 5>>(nodeCnt);
...
void output(const string &file_name, int pid)
{
    ...
    char *buf = (char *)mmap(NULL, result_length, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
    char *start;
    if (pid == 0)
    {
...
int main()
{
    ...
    // system("pause");
    Ans = vector<array<Loc, 5>>(nodeCnt);
