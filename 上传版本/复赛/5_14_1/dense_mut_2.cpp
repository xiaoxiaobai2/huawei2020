...
class Check {
  public:
    ...
    int maxC, minC;
    // 0  1   2   3``4    5
    vector<array<int, 7>> d1; // c1   min v1     v1
    vector<array<int, 7>> d2; // c1 c2  min v1 v2  v2
    vector<array<int, 7>> d3; // c1 c3  min v1 v2  v3
    vector<array<int, 7>> d4; // c1 c4  min v1 v2  v3 v4
    ...
    void readInput(string &testFile) {
        ...
        distruWork();
        t = clock() - t;
        // cout << "get denses and distrbute work spend : " << t << endl;
    }
    ...
    /*
        ...
        vector<array<int, 6>> d3; // c1 c4  min v1 v2  v3 v4
    */
    int k1, k2, k3, k4;
                ...
                if (v2.v != node && COK(v1.c, v2.c)) {
                    ...
                    for (VC &v3 : Gout[v2.v]) {
                        if (v3.v != node && v3.v != v1.v && COK(v2.c, v3.c)) {
                            ...
                            for (VC &v4 : Gout[v3.v]) {
                                if (v4.v != node && v4.v != v2.v &&
                                    v4.v != v1.v && COK(v3.c, v4.c)) {
                                    temp[1] = v4.c;
                                    temp[6] = v4.v;
                                    temp[2] = min(xiao2, temp[6]);
                                    d4.push_back(temp);
                                }
                            }
                        }
    ...
    void denseBack() {
        for (int i = k2; i < d2.size(); i++)
            if (head > d2[i][3])
                ...
                break;
        for (int i = k2; i < d2.size(); i++) {
            if (head < d2[i][2] && COK(headC, d2[i][0]) && able3[d2[i][4]]) {
                path[2] = d2[i][3];
                path[3] = d2[i][4];
                vis[d2[i][3]] = true;
                denseBackhome(d2[i][4], 2, d2[i][1]);
                vis[d2[i][3]] = false;
            }
        }
    ...
    */
        for (int i = k3; i < d3.size(); i++)
            if (head > d3[i][3])
                ...
                break;
        for (int i = k3; i < d3.size(); i++) {
            if (head < d3[i][2] && COK(headC, d3[i][0]) && able3[d3[i][5]]) {
                path[2] = d3[i][3];
                path[3] = d3[i][4];
                path[4] = d3[i][5];
                vis[d3[i][3]] = true;
                vis[d3[i][4]] = true;
                denseBackhome(d3[i][5], 3, d3[i][1]);
                vis[d3[i][3]] = false;
                vis[d3[i][4]] = false;
            }
        }
        for (int i = k4; i < d4.size(); i++)
            if (head > d4[i][3])
                k4++;
            else
                break;
        for (int i = k4; i < d4.size(); i++) {
            if (head < d4[i][2] && COK(headC, d4[i][0]) && able3[d4[i][6]]) {
                path[2] = d4[i][3];
                path[3] = d4[i][4];
                path[4] = d4[i][5];
                path[5] = d4[i][6];
    ...
    void denseBackhome(int cur, int depth, int c) {
        string tmp = "";
