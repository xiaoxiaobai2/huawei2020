...
vector<array<long long, 2>> CountIn, CountOut;
vector<string> ids;
int nodeCnt = 0;
...
class Check {
  public:
    ...
    void readInput(string &testFile) {
        ...
        CountIn = vector<array<long long, 2>>(nodeCnt);
        CountOut = vector<array<long long, 2>>(nodeCnt);
        array<long long, 2> tem;
        for (int i = 0; i < nodeCnt; i++) {
            int ma = -1, mi = 999999;
            for (VC &u : Gin[i]) {
                ma = max(ma, u.c);
    ...
    void dfs() {
        ...
        for (VC &v : Gout[head]) {
            ...
            path[1] = v.v;
            // 找3环
            // if (CountIn[head][0] > 3 * headC || CountIn[head][1] * 5 < headC)
            // {
            //     // cout << "0000" << endl;
            ...
            // dfs(v.v, 2, headC);
            if (CountOut[v.v][0] > 3 * headC || CountOut[v.v][1] * 5 < headC) {
                continue;
            }
            vis[v.v] = true;
            for (VC &u : Gout[v.v]) {
                ...
                if (CountOut[u.v][0] > 3 * u.c || CountOut[u.v][1] * 5 < u.c) {
                    // cout << "111111111" << endl;
                    continue;
                }
                vis[u.v] = true;
                for (VC &w : Gout[u.v]) {
                    ...
                    if (able4[w.v])
                        backhome(w.v, 3, w.c);
                    if (CountOut[w.v][0] > 3 * w.c ||
                        CountOut[w.v][1] * 5 < w.c) {
